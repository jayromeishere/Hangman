...
class Hangman
  ...
  def save_game(saved_board, saved_word, saved_turns, saved_incorrect_guesses)
    game = HangmanSaveGame.new(saved_board, saved_word, saved_turns, saved_incorrect_guesses)
    yaml = YAML::dump(game)
...
class HangmanGame < Hangman
  attr_reader :word
  attr_accessor :board, :turns, :incorrect_guesses
...
#    incorrect_guesses = []
    ...
    while turns > 0
      ...
      if guess == 'save'
        ...
        saved_word = @word
        saved_turns = @turns
        saved_incorrect_guesses = @incorrect_guesses
        save_game(saved_board, saved_word, saved_turns, saved_incorrect_guesses)
        Hangman.new.new_game?
...
class HangmanSaveGame < HangmanGame
  attr_accessor :board, :word, :turns, :incorrect_guesses
  
  def initialize(saved_board, saved_word, saved_turns, saved_incorrect_guesses)
    @board = saved_board
    @word = saved_word
    @turns = saved_turns
    @incorrect_guesses = saved_incorrect_guesses
  end
